...
export default function RegisterForm() {
  ...
  const onSubmit = async (data) => {
    ...
    if (res.token) {
      ...
      // echar un vistazo como se puede almacenar en las cookies en vez de en el localstorage
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
      <div className="flex-1 rounded-lg bg-gray-50 px-6 pb-4 pt-8">
        <h1 className={"mb-3 text-2xl text-black"}>Crea tu cuenta</h1>
      ...
      <div className="mt-6 text-white">
        <Link href="/login">
          ¿Ya tienes cuenta? <span className="text-blue-500">Accede</span>
        </Link>
      </div>
    </form>
  );
